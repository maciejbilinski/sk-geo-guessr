...
void Game::gameLoop()
{
    ...
    case GameState::INTRO:
        if (this->players_queue.size() >= CONFIG::MIN_PLAYERS_TO_VOTE)
        {
    ...
    case GameState::VOTING:
        ...
        { // dodajemy graczy i dajemy im szanse glosowac
            ...
            this->time_counter += CONFIG::VOTING_NEW_TIME;
            if (this->time_counter > CONFIG::VOTING_TIME)
                this->time_counter = CONFIG::VOTING_TIME;
        }
        break;
    }
...
{ // host wysłał nowe miejsce na czas, gracze odpowiadaaj
    ...
    (this->time_counter) = CONFIG::GAME_TIME;
    this->round++;
}
...
void Game::_addPlayer(Client *cl, bool inQueue)
{
    ...
    {
        if (this->players_queue.size() >= (CONFIG::MIN_PLAYERS_TO_VOTE-1)){
            this->backToVoting(false);
        }
    }
